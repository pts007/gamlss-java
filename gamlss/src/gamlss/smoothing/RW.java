 ...
 *
 */
public class RW implements GAMLSSSmoother {
	/** */
...
//-------------------------------------------------------  FREEZES HERE	
		...
		fv = (ArrayRealVector) solver.solve(wM.scalarMultiply(
										  FastMath.exp(-logsig2e)).operate(y));
...
//System.out.println(logsig2e+"   "+logsig2b +"   "+qF(new double[]{logsig2e, logsig2b}, y, w));					
				...
				nonLinObj.setWeights(w);
				 ...
				 bounds = new SimpleBounds(new double[] {-20.0, -20.0}, 
...
//System.out.println(logsig2e+"   "+logsig2b +"   "+values.getValue());	
	...
	class NonLinObjFunction implements MultivariateFunction {
		...
		public double value(final double[] point) {
			return qF (point, response, weights);
		}
	...
	class NonLinObjFunction1 implements MultivariateFunction {
		...
		public double value(final double[] point) {
			return qF1 (point, logsig2e, response, weights);
		}
	...
	class NonLinObjFunction2 implements MultivariateFunction {
		...
		public double value(final double[] point) {
			return qF2 (point, logsig2b, response, weights);
		}
	 ...
	 */
	//Qf <- function(par)
	private double qF1 (double[] par, double logsig2e, 
								ArrayRealVector y, ArrayRealVector w) {
		
		return qF(new double[]{logsig2e, par[0]}, y, w);
	}
	 ...
	 */
	//Qf <- function(par)
	private double qF2 (double[] par, double logsig2b, 
								ArrayRealVector y, ArrayRealVector w) {
		
		return qF(new double[]{par[0], logsig2b}, y, w);
	}
	...
	private double qF (double[] par, ArrayRealVector y, ArrayRealVector w) {
	    ...
	    solver = new QRDecomposition(wM.scalarMultiply(
	    				FastMath.exp(-par[0])).add(gM.scalarMultiply(
	    								  FastMath.exp(-par[1])))).getSolver();
	
		fv = (ArrayRealVector) solver.solve(
						  wM.scalarMultiply(FastMath.exp(-par[0])).operate(y));
		
		//b <- diff(fv, differences=order)
	    b = MatrixFunctions.diffV(fv, Controls.RW_ORDER);

	    //DT <-  determinant((exp(-par[1])*W) + (exp(-par[2])*G))$modulus
	    double dt = new LUD(wM.scalarMultiply(
	    				FastMath.exp(-par[0])).add(gM.scalarMultiply(
	    							FastMath.exp(-par[1])))).getDeterminant();
